...
function Maps() {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      const marble = res.Capability.Layer.Layer.find(
        (layer) => layer.Name === "bluemarble"
      );
        ...
        controls: defaultControls().extend([
          new MousePosition({
            coordinateFormat: (coordinate) => {
              return `Coordinates: ${coordinate[0].toFixed(
                2
  ...
  const handleAddLayer = () => {
    const check =
      isNotEmpty(layerUrl) ||
      validateXYZUrl(layerUrl) ||
      validateWMSUrl(layerUrl);
    setIsValid(check);
    let layerToAdd;
    if (!check) {
  ...
  return (
    <>
      ...
      <button className="menu-btn" onClick={toggleBottomBar}>
        {expanded ? "Hide" : "Import"}{" "}
      </button>
